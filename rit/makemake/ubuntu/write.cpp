/* write.cpp
 *
 * $Id: write.cpp,v 1.5 2011/05/27 02:47:43 sidney Exp $
 *
 * $Log: write.cpp,v $
 * Revision 1.5  2011/05/27 02:47:43  sidney
 * Fixed additions for .S files
 *
 * Revision 1.4  2011/05/26 03:44:56  sidney
 * Added .S file capability
 *
...
bool stemCompare(const string &x, const string &y) {
  return x.substr(0, x.rfind('.')) < y.substr(0, y.rfind('.'));
}
...
Write::Write( DirList &dirlist, bool noDebugFlag, bool useGCC,
  ...
  // fix sets
  set<string> temp;
  set_difference(dirlist.ass.begin(), dirlist.ass.end(),
                 dirlist.preass.begin(), dirlist.preass.end(),
                 inserter(temp, temp.end()), stemCompare);
  dirlist.ass = temp;
  temp.clear();
  set_difference(dirlist.ass_main.begin(), dirlist.ass_main.end(),
                 dirlist.preass.begin(), dirlist.preass.end(),
                 inserter(temp, temp.end()), stemCompare);
  dirlist.ass = temp;
  temp.clear();
  set_difference(dirlist.ass_other.begin(), dirlist.ass_other.end(),
                 dirlist.preass.begin(), dirlist.preass.end(),
                 inserter(temp, temp.end()), stemCompare);
  dirlist.ass = temp;
  // write makefile
...
void Write::write_header(bool noDebugFlag, bool useGCC, string header_file) {
  ...
  cout << "Ubuntu " __DATE__ ") on " << ctime( &now );
  cout << "#\n";
  cout << "\n";
  cout << "#\n";
  cout << "# Definitions\n";
...
void Write::write_lists( DirList &dirlist ){
  ...
  cout << "PS_FILES =\t" << FileListInserter(dirlist.preass) << '\n';
  ...
  cout << FileListInserter( dirlist.preass_other, ".o" );
  if( !dirlist.preass_other.empty() ) {
    cout << " ";
  }
...
void Write::write_main_targets( DirList &dirlist, string programs_file ){
  ...
  if( dirlist.cpp_main.empty() && dirlist.c_main.empty()
      && dirlist.preass_main.empty() && dirlist.ass_main.empty() ) {
  ...
  if(in) { // read lines of "programs.mak"
...
#ifdef WINDOWS
    cout << FileListInserter( dirlist.preass_main, ".exe" );
#else
    cout << FileListInserter( dirlist.preass_main, "" );
#endif
    if( !dirlist.preass_main.empty() ) {
      cout << " ";
    }
#ifdef WINDOWS
    ...
    write_main_target_list( dirlist.preass_main,
                            "$(CC) $(CFLAGS)" ,
                            local_libs + " $(CLIBFLAGS)");
...
void Write::write_dependencies( DirList &dirlist ){
  ...
  write_dependency_list( dirlist.preass, dirlist );
  cout << '\n';
}
...
void Write::write_trailer( DirList &dirlist, bool useGCC ){
  ...
  if( !dirlist.preass_main.empty() ){
    cout << " " << FileListInserter( dirlist.preass_main, ".o" );
  }
