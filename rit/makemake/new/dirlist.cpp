...
DirList::DirList( int ac, char **av ){
  ...
  if( dir != 0 ){
    ...
    while( ( dirent = readdir( dir ) ) != 0 ) {
      ...
      case CPP:
        if( ac <= 0 ) {
          insertSource( hasMainFunction, name, cpp, cpp_main, cpp_other );
        }
        break;

      case C:
        if( ac <= 0 ) {
          insertSource( hasMainFunction, name, c, c_main, c_other );
        }
        break;

      case ASSEMBLY:
        if( ac <= 0 ) {
          insertSource( hasMainLabel, name, ass, ass_main, ass_other );
        }
        break;
 ...
 * 'both' list, too.
 */
void DirList::insertSource( bool (* mainfn)( const string &name ),
                             const string &name,
                             set<string> &both,
                             set<string> &main,
                             set<string> &other ) {
...
DirList::FileType DirList::fileType( const string &name ) const {
  if( name[ 0 ] == '.' ) return OTHER; // "dot" file
  size_t pos = name.rfind('.'); // no suffix
 ...
 * first unmatched character or the next character after the matched
 * string if the string is completely matched.
 */
bool DirList::matchString( ifstream &in, char &ch, const char *s ) {
  for( int i = 0; s[i] != 0 && in; i++, in.get( ch ) ) {
    if( s[i] != ch ) {
      return false;
    }
  }
  return (bool)in; // true if stream still good else false for no match
}
